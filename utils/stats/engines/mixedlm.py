...
def _blocked_payload(
    *,
    ...
    return {
        ...
        "blocking_reasons": blocking_reasons,
        "suggested_actions": suggested_actions,
        "metadata": {"used_formula": used_formula, "time_order": time_order or [], "effect_sizes": None, "model_summary": None},
    }
...
def _fit_mixedlm(model):
    ...
    for kwargs in ({"reml": False, "method": "lbfgs", "disp": False}, {"reml": False, "method": "powell", "disp": False}):
        try:
...
def _build_contrast_table(
    ...
    if reference_group and reference_group not in group_levels:
        warnings.append(f"Reference group '{reference_group}' was not found in the fitted data. Falling back to '{reference_group_used}'.")
    ...
    return pd.DataFrame(rows), warnings, reference_group_used
